...
import random
import shutil
from I_configuration import TRAIN_SET_RATIO, TEST_SET_RATIO


class SplitDataSet:
    def __init__(self, dataset_dir, saved_dataset_dir,
                 train_ratio=TRAIN_SET_RATIO, test_ratio=TEST_SET_RATIO,
                 show_progress=False):
    ...
    def __get_all_file_path(self):
        all_file_path = []
        for idx, file_type in enumerate(self.__get_label_names()):
            self.idx_label_dic[idx] = file_type
    ...
    def __split_dataset(self):
        ...
        for index in range(len(all_file_paths)):
            ...
            self.train_file_path.append([self.idx_label_dic[index], file_path_list[: train_num]])
            self.test_file_path.append([self.idx_label_dic[index], file_path_list[train_num:train_num + test_num]])
            self.valid_file_path.append([self.idx_label_dic[index], file_path_list[train_num + test_num:]])
...
if __name__ == '__main__':
    split_dataset = SplitDataSet(dataset_dir="A_original_dataset",
                                 saved_dataset_dir="B_dataset",
                                 show_progress=True)
